...
### 6.4 Confirmed: What the Inbound Status-Callback Payload Actually Sends (`shippingVendor` present, `trackingUrl`/label absent)
A related question was raised: *does the inbound event-handler/TIO-side status-update payload send us the carrier name or a tracking URL?* Confirmed from code — `POST v1/returnstatus` (`ReturnStatusController.java`, documented as "Call Update-Status api from Event Handler service"), bound to `ReturnStatusDTO` → mapped to `ReturnStatusS4`:
- **`shippingInfo[]` array fields (both `ReturnStatusDTO.ShippingInfoDTO` and `ReturnStatusS4.ShippingInfo`):** `shippingVendor`, `trackingNumber`, `partSerialNumber`, `partLineNum` — confirmed from actual documented sample payloads, e.g. `{"partLineNum":"1","partSerialNumber":"CMK2460P29","shippingVendor":"FEDEX","trackingNumber":"000000002312"}`.
- **`shippingVendor` IS sent inbound** — as a plain string (e.g. `"FEDEX"`), not an enum. For DHL, this would simply carry `"DHL"` instead — **no schema change needed on this inbound contract.**
- **`trackingUrl` is NOT sent inbound** — no such field exists anywhere in `ReturnStatusDTO`/`ReturnStatusS4`/`ShippingInfo`. SRS never receives a tracking URL from this callback; it only ever *constructs* one outbound from the tracking number (4.1). This is consistent, not a gap.
- **No label/`labelURL` field exists inbound either** — confirmed absent from this same model, consistent with 7.1's finding that label data only ever flows on the outbound carrier-creation call, never on this inbound status callback.
- **Conclusion:** the inbound status-callback contract is already carrier-neutral for the fields it does carry (`shippingVendor`, `trackingNumber`) — introducing `"DHL"` as a `shippingVendor` value requires no DTO/schema change here. This closes out 11 S4/EMS Team question about whether this inbound callback needs a tracking URL or label field added — it doesn't, because it never had one for FedEx either.

### 6.5 Confirmed: The Immediate Synchronous Response to "Generate Return Label" ALSO Carries `shippingVendor` — This Is How the Caller Identifies the Carrier
A question was raised: *when TAO (or whichever system) calls SRS to generate a return label, does the response we send back indicate which carrier was used — how would they even know it's DHL vs. FedEx?* Confirmed from code — this is a **third, separate contract surface** from 6.3 (inbound request) and 6.4 (inbound status callback):
- `POST v1/internal/initiate` (`ReturnDetailsController.sendReturn()`) returns `ReturnResponse` **synchronously**, in the same HTTP response to the caller that requested the label.
- **`ReturnResponse.java` fields include:** `subscriptionId`, `returnOrderId`, `fulfillmentOrderId`, `items`, `status`, `timeStamp`, `returnReason`, `trackingNumber`, `trackingUrl`, **`shippingVendor`** (plain `String`, example value `"None"` in the schema doc), `timeoutInDays`, `history`, `triggerNumber`, `returnType`, `hasOrderAttributionId`, `isWriteOffReturn`.
- **This is exactly how the caller identifies the carrier today** — `shippingVendor` is present in this very first synchronous response, not just in the later async EMS event (9a) or the inbound status callback (6.4). All three surfaces carry the same plain-string field name.
- **For DHL:** this field would simply carry `"DHL"` instead of `"FEDEX"`/`"Fedex"` — **no schema/DTO change needed**, since `shippingVendor` is already a carrier-neutral `String`, not an enum, everywhere in this codebase.
- **What this means for the caller (TAO or whichever system consumes this):** if that caller has any client-side logic that branches on the literal string value of `shippingVendor` (e.g., `if ("FEDEX".equals(vendor))`), that logic would need updating on **their side** to also recognize `"DHL"` as a valid value — this is a caller-side integration note to communicate, not an SRS-side gap. This is the same underlying open question as 11 S4/EMS Team item #16 ("are there any downstream systems hardcoded to assume `Fedex` as the only carrier value") — now confirmed to apply to **three** surfaces (this synchronous response, the async EMS event, and the inbound status callback), not just one.
...
16. Are there any downstream systems hardcoded to assume `"Fedex"` as the only carrier value? **Now confirmed to matter across THREE separate surfaces, not one:** (a) the synchronous `ReturnResponse` returned immediately from `POST v1/internal/initiate` (6.5), (b) the async outbound EMS event (9a/Appendix A #9), and (c) the inbound status-callback `v1/returnstatus` (6.4) — all three carry `shippingVendor` as a plain string today; any caller hardcoded to only recognize `"Fedex"`/`"FEDEX"` on any of these three surfaces needs updating on **their** side to also accept `"DHL"`.
16a. ~~Does the inbound status-callback (`v1/returnstatus`) need a new `trackingUrl` or label field added for DHL?~~ **RESOLVED (6.4):** No — that inbound contract never carried a `trackingUrl` or label field for FedEx either; it only ever carries `shippingVendor` + `trackingNumber`, both already plain carrier-neutral strings. `"DHL"` slots in as a value with zero schema change.
...
| Inbound status-callback contract (`v1/returnstatus`) | `shippingInfo[].shippingVendor` + `.trackingNumber` only — no `trackingUrl`/label field ever existed | Same fields, `shippingVendor` value becomes `"DHL"` | None | None — already carrier-neutral (6.4) | CONFIRMED (no gap) | N/A |
| Synchronous "generate label" response (`ReturnResponse`) | Carries `shippingVendor` (+ `trackingNumber`, `trackingUrl`) back to the caller in the same HTTP response | Same field, value becomes `"DHL"` | None on SRS side; caller-side hardcoding risk only | Communicate new `"DHL"` value to whichever team consumes this response (6.5) | CONFIRMED (no SRS-side gap; caller-side check needed) | SRS team → caller team |
...
| 19 | `api/ReturnStatusController.java:59-65` | Inbound `POST v1/returnstatus` — "Call Update-Status api from Event Handler service"; documented sample payloads confirm `shippingInfo[]` shape |
| 20 | `api/dto/ReturnStatusDTO.java:69-74` | `ShippingInfoDTO` — fields: `shippingVendor`, `trackingNumber`, `partSerialNumber`, `partLineNum` only; no `trackingUrl`/label field |
| 21 | `api/ReturnDetailsController.java:59-63` | `POST v1/internal/initiate` → `sendReturn()` — returns `ReturnResponse` synchronously in the same HTTP response |
| 22 | `api/dto/ReturnResponse.java:53-63` | `ReturnResponse` — confirms `trackingNumber`, `trackingUrl`, `shippingVendor` (plain `String`) are present in the immediate synchronous response, not just the async EMS event |
